...
class SemanticClassifier:
    ...
    def fit(
        ...
        if do_calibration:
            logger.info("Applying isotonic probability calibration...")
            # Create a clone of base_model without early_stopping_rounds for calibration
            # CalibratedClassifierCV uses CV which doesn't support eval_set
            calibration_model = xgb.XGBClassifier(
                ...
                min_child_weight=self.base_model.min_child_weight,
                subsample=self.base_model.subsample,
                colsample_bytree=self.base_model.colsample_bytree,
                reg_alpha=self.base_model.reg_alpha,
                reg_lambda=self.base_model.reg_lambda,
                random_state=self.base_model.random_state,
                n_jobs=self.base_model.n_jobs,
                eval_metric=self.base_model.eval_metric,
                # Note: early_stopping_rounds is intentionally omitted
                # because CalibratedClassifierCV doesn't support eval_set
            )
            # Copy additional attributes if they exist
            if hasattr(self.base_model, 'gamma'):
                calibration_model.gamma = self.base_model.gamma
            if hasattr(self.base_model, 'scale_pos_weight'):
                calibration_model.scale_pos_weight = self.base_model.scale_pos_weight
