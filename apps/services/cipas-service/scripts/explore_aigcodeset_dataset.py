#!/usr/bin/env python3
"""
Example script showing how to use the downloaded AIGCodeSet dataset.
...
def load_dataset(dataset_dir: Path, logger):
    """
    Load the AIGCodeSet dataset from CSV files.
    ...
    if not dataset_dir.exists():
        ...
        logger.info("Please run download_aigcodeset_dataset.py first to download the dataset")
        return None
    ...
    for split_name, df in datasets.items():
        ...
        for idx, row in df.head(2).iterrows():
            ...
            for col, value in row.items():
                if col == 'code' or col == 'content' or 'code' in col.lower():
...
def analyze_code_samples(datasets: dict, logger):
    ...
    for split_name, df in datasets.items():
        # Find code column (could be 'code', 'content', etc.)
        code_col = None
        ...
        java_patterns = df[code_col].astype(str).str.contains('public class|import java|System\.', case=False, na=False).sum()
        python_patterns = df[code_col].astype(str).str.contains('def |import |from .* import', case=False, na=False).sum()
...
def main():
    ...
    dataset_dir = project_root / "datasets" / "aigcodeset"
    
    logger.info("AIGCodeSet Dataset Usage Example")
    logger.info("="*50)
