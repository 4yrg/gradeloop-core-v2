#!/usr/bin/env python3
"""
Example script showing how to use the downloaded HumanVsAICode dataset.
...
def load_dataset(dataset_dir: Path, logger):
    """
    Load the HumanVsAICode dataset from CSV files.
    ...
    if not dataset_dir.exists():
        ...
        logger.info("Please run download_humanvsai_dataset.py first to download the dataset")
        return None
...
def analyze_code_samples(datasets: dict, logger):
    ...
    for split_name, df in datasets.items():
        ...
        line_counts = df['code'].astype(str).str.count('\n') + 1
        ...
        java_patterns = df['code'].astype(str).str.contains('public class|import java', case=False, na=False).sum()
        python_patterns = df['code'].astype(str).str.contains('def |import |from .* import', case=False, na=False).sum()
        c_patterns = df['code'].astype(str).str.contains('#include|int main', case=False, na=False).sum()
        js_patterns = df['code'].astype(str).str.contains('function |const |let |var ', case=False, na=False).sum()
...
def main():
    ...
    logger.info("HumanVsAICode Dataset Usage Example")
    logger.info("="*50)
