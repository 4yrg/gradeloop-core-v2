#!/usr/bin/env python3
"""
Script to download the AIGCodeSet dataset from HuggingFace Hub.
Dataset: https://huggingface.co/datasets/basakdemirok/AIGCodeSet
...
def download_dataset(output_dir: Path, logger):
    """
    Download the AIGCodeSet dataset from HuggingFace Hub.
    
    Args:
        output_dir (Path): Directory to save the dataset
        logger: Logger instance
    """
    logger.info("Starting download of AIGCodeSet dataset...")
    ...
    try:
        ...
        for split_name, split_data in dataset.items():
            ...
            if 'label' in df.columns:
                ...
                logger.info(f"{split_name} split human/AI distribution:")
                for label, count in label_counts.items():
        ...
        with open(info_path, 'w') as f:
            f.write("AIGCodeSet Dataset Information\n")
            f.write("="*40 + "\n\n")
            f.write(f"Source: https://huggingface.co/datasets/basakdemirok/AIGCodeSet\n")
