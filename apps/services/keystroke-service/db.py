...
class DatabaseClient:
    ...
    def save_template(self, user_id: str, phase: str, template: np.ndarray, 
                     template_std: np.ndarray = None, sample_count: int = 1,
        ...
        try:
            ...
            with self.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    """, (user_id, phase, template_bytes, std_bytes, sample_count, 
                ...
                with conn.cursor(cursor_factory=extras.RealDictCursor) as cursor:
                    ...
                    for row in rows:
                        templates[row['enrollment_phase']] = {
                            ...
                            'std': pickle.loads(row['template_std']) if row['template_std'] else None,
    ...
    def log_auth_event(self, user_id: str, session_id: str, offset_seconds: int,
                      similarity_score: float, risk_score: float, authenticated: bool,
                      assignment_id: str = None, course_id: str = None,
                      anomaly_type: str = None, is_struggling: bool = False,
                      matched_phase: str = None, metadata: Dict = None) -> bool:
        """
        ...
        try:
            ...
            with self.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    """, (user_id, session_id, assignment_id, course_id, offset_seconds,
                          similarity_score, risk_score, authenticated, confidence,
                          is_anomaly, anomaly_type, is_struggling, matched_phase, metadata_json))
    ...
    def archive_session(self, user_id: str, session_id: str, events: List[Dict],
                       assignment_id: str = None, course_id: str = None,
                       final_code: str = None, behavioral_analysis: Dict = None,
        ...
        try:
            ...
            with self.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    """, (user_id, session_id, assignment_id, course_id, json.dumps(events),
                          len(events), duration, avg_risk, max_risk, anomaly_count,
                          failure_count, final_code, json.dumps(behavioral_analysis),
                          retention_until))
