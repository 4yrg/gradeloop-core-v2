...
GRADIENT_ACCUMULATION_STEPS = (
    2  # Gradient accumulation (Effective batch = BATCH_SIZE * STEPS)
)
...
def optimize_for_rtx6000(logger: logging.Logger):
    ...
    if torch.cuda.is_available():
        ...
        logger.info(
            "✓ RTX 6000 Blackwell optimizations enabled (TF32, cuDNN benchmark)"
        )
...
class DatasetLoader:
    ...
    GPTCLONEBENCH_GDRIVE_URL = (
        f"https://drive.google.com/uc?id={GPTCLONEBENCH_GDRIVE_ID}"
    )
    ...
    def __init__(self, datasets_dir: Path, logger: logging.Logger, max_samples: int):
        ...
        self.gptclonebench_path = (
            datasets_dir / "gptclonebench" / "gptclonebench_dataset.jsonl"
        )
                    ...
                    pairs.append(
                        {
                            "code1": data["code1"],
                            "code2": data["code2"],
                            "label": 1 if data["semantic"] else 0,
                            "source": "gptclonebench",
                        }
                    )
    ...
    def create_codenet_pairs(
        self, problems: Dict[str, List[Dict]], num_positive: int, num_negative: int
    ) -> List[Dict[str, Any]]:
        pairs = []
        ...
        for pid in problem_ids:
            ...
            sampled = random.sample(
                list(combinations(subs, 2)),
                min(sample_size, len(subs) * (len(subs) - 1) // 2),
            )
    ...
    def load_and_balance(self) -> List[Dict[str, Any]]:
        ...
        code_probs = self.load_codenet_submissions(
            self.codenet_path, limit=code_samples * 3
        )
        code_pairs = (
            self.create_codenet_pairs(code_probs, code_pos, code_neg)
            if code_probs
            else []
        )
        ...
        self.logger.info(
            f"✓ Dataset: {len(all_pairs)} pairs, {sum(1 for p in all_pairs if p['label'] == 1)} clones"
        )
        return all_pairs
...
class SemanticCloneDataset(Dataset):
    def __init__(self, pairs, tokenizer, max_length, augmenter=None):
        self.pairs, self.tokenizer, self.max_length, self.augmenter = (
            pairs,
            tokenizer,
            max_length,
            augmenter,
    ...
    def __getitem__(self, idx):
        ...
        return {
            "input_ids1": e1["input_ids"].squeeze(0),
            "attention_mask1": e1["attention_mask"].squeeze(0),
            "input_ids2": e2["input_ids"].squeeze(0),
            "attention_mask2": e2["attention_mask"].squeeze(0),
...
class EarlyStopping:
    def __init__(self, patience=5, mode="max"):
        self.patience, self.mode, self.counter, self.best, self.stop = (
            patience,
            mode,
            0,
            None,
    ...
    def __call__(self, score):
        if self.best is None:
            self.best = score
            return False
        if (self.mode == "max" and score > self.best) or (
...
class MetricsSaver:
    ...
    def save_final_evaluation(self, metrics, labels, preds, probs, ep):
        ...
        pd.DataFrame({"true": labels, "pred": preds, "prob": probs}).to_csv(
            self.m_dir / f"preds_{ts}.csv", index=False
        )
...
class Trainer:
    def __init__(
        ...
        self.model, self.train_ld, self.val_ld, self.test_ld = (
            model,
            train_ld,
            val_ld,
            test_ld,
    ...
    def train_epoch(self, ep):
        ...
        for step, b in enumerate(tqdm(self.train_ld, desc=f"Ep {ep}")):
            i1, m1, i2, m2, labels = (
                b["input_ids1"].to(self.dev),
                b["attention_mask1"].to(self.dev),
                b["input_ids2"].to(self.dev),
                b["attention_mask2"].to(self.dev),
                b["labels"].to(self.dev),
            )
            if USE_MIXED_PRECISION:
                with autocast():
                    loss = (
                        self.crit(self.model(i1, m1, i2, m2), labels)
                        / GRADIENT_ACCUMULATION_STEPS
                    )
    ...
    def evaluate(self, ld, desc, ep, split="val"):
        ...
        if split == "val" and ep > 0:
            self.saver.save_confusion_matrix(
                np.array(all_l), np.array(all_p), ep, split
            )
        return tracker.compute()
        ...
        if checkpoint_path and checkpoint_path.exists():
            ...
            if best_ckpt.exists():
                ...
                if ckpts:
                    ...
                    self.logger.warning(
                        f"⚠️ best_model.pt not found! Using latest: {selected_ckpt.name}"
                    )
                else:
...
def parse_args():
    ...
    p.add_argument(
        "--mode",
        default="full",
        choices=["full", "resume", "evaluate", "download-only"],
    )
...
def main():
    ...
    saver = MetricsSaver(
        OUTPUT_DIR / "results" / "metrics", OUTPUT_DIR / "results" / "plots", logger
    )
    trainer = Trainer(
        model,
        tr_ld,
